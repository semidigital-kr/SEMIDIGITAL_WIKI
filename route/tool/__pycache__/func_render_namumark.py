    ...
    def do_render_link(self):
        ...
        while 1:
            ...
            if not link_data:
                ...
                #나무위키 링크 표시 문법 (by SEMIDIGITAL)
                elif link_main.startswith("https://namu.wiki"):
                    # '|'를 기준으로 링크와 표시할 문자를 분리
                    if "|" in link_main:
                        link_main, link_display = link_main.split("|", 1)  # 링크와 표시할 문자 분리
                    else:
                        link_display = link_main  # '|'가 없으면 링크 자체를 표시할 문자로 사용

                    # 링크 처리
                    link_main = self.get_tool_data_restore(link_main, do_type='link')  # link_main은 링크 주소
                    link_title = link_display  # 표시할 문자는 link_display

                    # 내부 링크처럼 처리
                    link_class = "opennamu_link"  # 내부 링크 스타일로 처리
                    add_str = ''  # 추가적인 문자열

                    # link_data_full을 이곳에서 처리
                    link_data_full = f'[[{link_main}|{link_display}]]'

                    # 중복을 피하기 위해 HTML로 이미 변환된 부분은 건너뜁니다.
                    if f'<a href="{link_main}"' not in self.render_data:
                        # 링크를 HTML로 렌더링 (일반적인 내부 링크처럼)
                        self.render_data = self.render_data.replace(f'[[{link_main}|{link_display}]]', f'<a href="{link_main}" class="{link_class}" title="{link_title}">{link_display}</a>')
